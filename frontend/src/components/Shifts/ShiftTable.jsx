...
export default function ShiftTable({ loading, shifts, currencySymbol, isAdmin }) {
    ...
    return (
        <div className="table-darkx border-0 overflow-visible">
            <table className="table table-borderless table-hover mb-0">
                <thead>
                    <tr className="border-bottom border-white-10">
                        ...
                        <th className="px-4 py-3 x-small fw-bold">START TIME</th>
                        <th className="px-4 py-3 x-small fw-bold">END TIME</th>
                ...
                <tbody>
                    {shifts.map((shift) => (
                        <tr key={shift.id} className="align-middle border-bottom border-white-5">
                            ...
                            {isAdmin && (
                                <td className="px-4 py-3">
                                    <div className="d-flex align-items-center gap-2">
                                        ...
                                        <span className="fw-medium small">{shift.userName}</span>
                                    </div>
                                </td>
                            )}
                            ...
                            <td className="px-4 py-3 small opacity-75">
                                {shift.endTime ? new Date(shift.endTime).toLocaleString() : '-'}
                            </td>
                            <td className="px-4 py-3 fw-bold">{currencySymbol}{parseFloat(shift.openingBalance).toFixed(2)}</td>
                            <td className="px-4 py-3">
                                {shift.closingBalance ? (
                                    <span className="fw-bold text-info">{currencySymbol}{parseFloat(shift.closingBalance).toFixed(2)}</span>
                                ) : '-'}
                            </td>
                            <td className="px-4 py-3 text-success fw-bold">
