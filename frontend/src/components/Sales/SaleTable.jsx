        ...
        <div className="table-darkx shadow-soft border-white-10">
            <div className="table-responsive">
                <table className="table table-borderless table-hover mb-0">
                    <thead>
                        <tr>
                            <th className="px-4 py-3 fw-bold small text-nowrap">#</th>
                            <th className="px-4 py-3 fw-bold small text-nowrap">DATE</th>
                            <th className="px-4 py-3 fw-bold small text-nowrap">CUSTOMER</th>
                            <th className="px-4 py-3 fw-bold small text-nowrap">TOTAL</th>
                            <th className="px-4 py-3 text-center fw-bold small text-nowrap">PAYMENT</th>
                            {!isCashierLike && <th className="px-4 py-3 fw-bold small text-nowrap">CASHIER</th>}
                            <th className="px-4 py-3 text-end fw-bold small text-nowrap">ACTIONS</th>
                        </tr>
                    </thead>
                    <tbody>
                        {loading && sales.length === 0 ? (
                            ...
                            sales.map((s, index) => (
                                <tr key={s.id} className="align-middle border-bottom border-white-5">
                                    <td className="px-4 py-3 text-muted">
                                        {pagination.total - ((pagination.page - 1) * pagination.limit) - index}
                                    </td>
                                    <td className="px-4 py-3 small text-muted text-nowrap">
                                    ...
                                    {!isCashierLike && <td className="px-4 py-3 small opacity-75 text-nowrap">{s.user_name}</td>}
